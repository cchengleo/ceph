...
void ObjectCacher::bh_read(BufferHead *bh, bool op_compression)
{
  assert(lock.is_locked());
...
int ObjectCacher::readx(OSDRead *rd, ObjectSet *oset, Context *onfinish,
                        bool op_compression)
{
  return _readx(rd, oset, onfinish, true, op_compression);
}

int ObjectCacher::_readx(OSDRead *rd, ObjectSet *oset, Context *onfinish,
			 bool external_call, bool op_compression)
{
  assert(lock.is_locked());
  bool success = true;
  int error = 0;
  ...
  for (vector<ObjectExtent>::iterator ex_it = rd->extents.begin();
	...
	if (!waitfor_read.empty() || rx_bytes > max_size) {
	  ...
	  bh_read(bh_it->second, op_compression);
	  if (success && onfinish) {
